...
function setUniqueMonthFilter(month) {
  ...
  const previousButtonMonth = document.getElementById(
    "timebutton" + activeFilters.time.start
  );
...
function swapTimeFilterMethod() {
  ...
  if (swapButton.innerHTML == "Buttons") {
    [
      ...
      (item) =>
        (document.getElementById("timebutton" + item).className = "time-button")
    );
...
async function applyFilters() {
  ...
  if (activeFilters.time.start != "" && activeFilters.time.end != "") {
    ...
    newData = newData.filter(function (row) {
      var month =
        new Date(parseDate(row["Start_Time"].slice(0, 7))).getMonth() + 1;
      return (
        month >= monthMap[activeFilters.time.start] &&
        month <= monthMap[activeFilters.time.end]
      );
    });
  }
  // Severity filter
...
function changeFilterAccordion() {
  ...
  if (node.innerHTML == "+") {
    ...
    node.innerHTML = "+";
    parentNode.className = "filters accordion-disabled";
  }
}

/**
 * Updates the classes in the basic visualization module to hide/show the panel with an accordion animation.
 * @param {string} type Refers to which of the 2 panels of the module needs to be updated.
 */
function changeAccordionBasicVis(panel) {
  const node = document.getElementById("basiccontainer" + panel);
  if (node.className == "basic-visualization accordion-disabled") {
    node.className = "basic-visualization";
  } else {
    node.className = "basic-visualization accordion-disabled";
  }
}
