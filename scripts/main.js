...
var weatherConditions = ['Light Rain', 'Overcast', 'Mostly Cloudy', 'Snow', 'Light Snow', 'Cloudy', '', 'Scattered Clouds', 'Clear', 'Partly Cloudy', 'Light Freezing Drizzle', 'Light Drizzle', 'Haze', 'Rain', 'Heavy Rain', 'Fair', 'Drizzle', 'Fog', 'Thunderstorms and Rain', 'Patches of Fog', 'Light Thunderstorms and Rain', 'Mist', 'Rain Showers', 'Light Rain Showers', 'Heavy Drizzle', 'Smoke', 'Light Freezing Fog', 'Light Freezing Rain', 'Blowing Snow', 'Heavy Thunderstorms and Rain', 'Heavy Snow', 'Snow Grains', 'Squalls', 'Light Fog', 'Shallow Fog', 'Thunderstorm', 'Light Ice Pellets', 'Thunder', 'Thunder in the Vicinity', 'Fair / Windy', 'Light Rain with Thunder', 'Heavy Thunderstorms and Snow', 'Light Snow Showers', 'Cloudy / Windy', 'Ice Pellets', 'N/A Precipitation', 'Light Thunderstorms and Snow', 'T-Storm', 'Rain / Windy', 'Wintry Mix', 'Partly Cloudy / Windy', 'Heavy T-Storm', 'Sand', 'Light Rain / Windy', 'Widespread Dust', 'Mostly Cloudy / Windy', 'Blowing Dust / Windy', 'Blowing Dust', 'Volcanic Ash', 'Freezing Rain / Windy', 'Small Hail', 'Wintry Mix / Windy', 'Light Snow / Windy', 'Heavy Ice Pellets', 'Heavy Snow / Windy', 'Heavy Rain / Windy', 'Heavy T-Storm / Windy', 'Fog / Windy', 'Dust Whirls', 'Showers in the Vicinity', 'Funnel Cloud', 'Haze / Windy', 'Light Rain Shower', 'Smoke / Windy', 'Light Drizzle / Windy', 'Snow / Windy', 'Partial Fog', 'Light Freezing Rain / Windy', 'Sleet', 'Blowing Snow / Windy', 'Snow and Sleet', 'Snow and Sleet / Windy', 'Squalls / Windy', 'Light Sleet / Windy', 'Freezing Drizzle', 'Freezing Rain', 'Thunder / Windy', 'Drizzle and Fog', 'Light Sleet', 'Thunder / Wintry Mix / Windy', 'Mist / Windy', 'Light Snow and Sleet', 'Rain Shower', 'Sleet / Windy', 'T-Storm / Windy', 'Light Snow and Sleet / Windy', 'Patches of Fog / Windy', 'Drizzle / Windy', 'Sand / Dust Whirls Nearby', 'Light Rain Shower / Windy', 'Heavy Rain Shower', 'Thunder and Hail', 'Drifting Snow', 'Light Snow Shower', 'Sand / Dust Whirlwinds', 'Heavy Blowing Snow', 'Hail', 'Heavy Freezing Drizzle', 'Low Drifting Snow', 'Light Blowing Snow', 'Heavy Rain Showers', 'Light Haze', 'Heavy Thunderstorms with Small Hail', 'Light Snow with Thunder', 'Heavy Snow with Thunder', 'Thunder and Hail / Windy', 'Tornado']
var promises = [d3.csv("data/main1000.csv")];
...
function init() {
  activeFilters = {
    ...
    causes: [],
    weatherConditions: [],
    humidity: { value: null, condition: 'x' },
    temperature: { value: null, condition: 'x' },
    visibility: { value: null, condition: 'x' },
    windspeed: { value: null, condition: 'x' },
    precipitation: { value: null, condition: 'x' }
  }
  const params = {
    data: currentData
  }
...
function initFiltersElements() {
  ...
  for (let i = 0; i < statesData.length; ++i) {
    ...
    input.onclick = function () { changeFilter('States', statesData[i].value) };
  ...
  for (let i = 0; i < weatherConditions.length; ++i) {
    ...
    input.onclick = function () { changeFilter('Weather Conditions', weatherConditions[i]) };
    var label = document.createElement("label")
...
function setNumericalFiltersMinAndMax() {
  const visibilityValues = data.map(function (row) { return row['Visibility(mi)'] });
  ...
  const windspeedValues = data.map(function (row) { return row['Wind_Speed(mph)'] });
  ...
  const precipitationValues = data.map(function (row) { return row['Precipitation(in)'] });
  document.getElementById('visibilitySlider').min = Math.min(precipitationValues)
  document.getElementById('visibilitySlider').max = Math.max(precipitationValues)
}
...
function changeFilter(type, value) {
  if (type == "Severity") {
    ...
    if (activeFilters.weatherConditions.includes(value)) activeFilters.weatherConditions = activeFilters.weatherConditions.filter(function (row) { return row != value });
...
function changeNumericalCondition(filtertype) {
  ...
  if (index == numericalConditionsOrder.length - 1) {
    ...
    node.className = index == numericalConditionsOrder.length - 2 ? "numerical-condition-button numerical-condition-button--off" : "numerical-condition-button"
...
function applyFilters() {
  ...
  for (let i = 0; i < activeFilters.weatherConditions.length; ++i) {
    newData = newData.filter(function (row) { return activeFilters.weatherConditions.includes(row['Weather_Condition']) });
  }

  const numericalConditions = ['temperature', 'visibility', 'humidity', 'windspeed', 'precipitation']
  for (let i = 0; i < numericalConditions.length; ++i) {
    if (activeFilters[numericalConditions[i]].condition != 'x') {
      ...
      switch (activeFilters[numericalConditions[i]].condition) {
        case '=':
          newData = newData.filter(function (row) {
            return row[numericalConditionsMap[numericalConditions[i]]] == Number(activeFilters[numericalConditions[i]].value)
          });
          break;
        case '&gt;':
          ...
          newData = newData.filter(function (row) {
            ...
            return parseInt(row[numericalConditionsMap[numericalConditions[i]]]) > Number(activeFilters[numericalConditions[i]].value)
          });
          console.log(newData)
          break;
        case '≥':
