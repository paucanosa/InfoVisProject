class GeoVis {
  constructor(params) {
    ...
    this.statesData.forEach(
      (state) => (this.stateMapping[state.value] = state.label)
    );
  ...
  updateSideChart() {
    ...
    if (this.selectedState != undefined) {
      var states = sideSvg
        .selectAll("path")
        .data(
          topojson.feature(this.geoData, this.geoData.objects.states).features,
          (d) => {
      ...
      states.join(
        (enter) => {
          return enter
            ...
            .merge(states)
            .attr("fill", (d) =>
              d.properties.name == this.selectedState
                ? "lightblue"
                : "rgb(240,240,240)"
            )
            .attr("d", path)
            .style("stroke", (d) =>
              d.properties.name == this.selectedState ? "grey" : "white"
            );
        },
        (update) => {
          return update;
      ...
      var dataForState = this.currentData.filter(
        (d) => this.stateMapping[d.State] == this.selectedState
      );
      ...
      var accidentPercentage =
        (parseFloat(this.points.length) / this.currentData.length) * 100;
      var dataForStateWithSeverity = dataForState.filter(
        (d) => d["Severity"] != ""
      );
      var totalSeverity = dataForStateWithSeverity
        .map((d) => d["Severity"])
        .reduce((a, b) => a + parseInt(b), 0);
      var averageSeverity =
        dataForStateWithSeverity.length > 0
          ? totalSeverity / dataForStateWithSeverity.length
          : 0;

      var dataForStateWithTemperature = dataForState.filter(
        (d) => d["Temperature(F)"] != ""
      );
      var totalTemperature = dataForStateWithTemperature
        .map((d) => d["Temperature(F)"])
        .reduce((a, b) => a + parseFloat(b), 0);
      var averageTemperature =
        dataForStateWithTemperature.length > 0
          ? totalTemperature / dataForStateWithTemperature.length
          : 0;

      var dataForStateWithHumidity = dataForState.filter(
        (d) => d["Humidity(%)"] != ""
      );
      var totalHumidity = dataForStateWithHumidity
        .map((d) => d["Humidity(%)"])
        .reduce((a, b) => a + parseFloat(b), 0);
      var averageHumidity =
        dataForStateWithHumidity.length > 0
          ? totalHumidity / dataForStateWithHumidity.length
          : 0;

      var dataForStateWithPrecipitation = dataForState.filter(
        (d) => d["Precipitation(in)"] != ""
      );
      var totalPrecipitation = dataForStateWithPrecipitation
        .map((d) => d["Precipitation(in)"])
        .reduce((a, b) => a + parseFloat(b), 0);
      var averagePrecipitation =
        dataForStateWithPrecipitation.length > 0
          ? totalPrecipitation / dataForStateWithPrecipitation.length
          : 0;

      var dataForStateWithWindSpeed = dataForState.filter(
        (d) => d["Wind_Speed(mph)"] != ""
      );
      var totalWindSpeed = dataForStateWithWindSpeed
        .map((d) => d["Wind_Speed(mph)"])
        .reduce((a, b) => a + parseFloat(b), 0);
      var averageWindSpeed =
        dataForStateWithWindSpeed.length > 0
          ? totalWindSpeed / dataForStateWithWindSpeed.length
          : 0;
      ...
      var censusStateInfo = this.censusData.filter(
        (dataPoint) => dataPoint["STATE"] == this.selectedState
      )[0];
      var statePopulation = parseInt(censusStateInfo["POPESTIMATE2019"]);
      var stateCapitalCoordinates = [
        censusStateInfo["long"],
        censusStateInfo["lat"],
      ];
      ...
      infoTexts = [
        ...
        [
          ...
          "Accidents per million inhabitants: " +
            parseInt(
              (accidentCount / statePopulation) * 1_000_000
            ).toLocaleString(),
        ],
        [
          "Accident percentage",
          "Accident % (regarding US total): " +
            (isFinite(accidentPercentage) ? accidentPercentage.toFixed(1) : 0) +
            "%",
        ],
        [
          "Delta from mean",
          "Delta from mean: " +
            parseInt(
              accidentCount - this.stateCountStats.absolute_mean
            ).toLocaleString(),
        ],
        [
          "Delta from max",
          "Delta from max: " +
            parseInt(
              accidentCount - this.stateCountStats.absolute_max
            ).toLocaleString(),
        ],
    ...
    if (this.selectedState != undefined) {
      var stateData = this.statesData.filter(
        (d) => d.label == this.selectedState
      )[0];

      translateLongLat = [stateData.longitude, stateData.latitude];
      translatePoint = [
        this.projection(translateLongLat)[0],
        this.projection(translateLongLat)[1] - 32,
      ];
      translateBounds = [
        [
          this.projection(translateLongLat)[0],
    ...
    var zoom = d3
      ...
      .on("zoom", (event) => {
        ...
        this.sideSvg.selectAll("path").attr("transform", event.transform);
        ...
        this.pointsLayer
          ...
          .attr(
            "r",
            "" + this.pointPixelSize / event.transform.k ** 0.4 + "px"
          );

        this.capitalPointsLayer
          .selectAll("circle")
          .attr("transform", event.transform)
          .attr(
            "r",
            "" + (this.pointPixelSize * 2) / event.transform.k ** 0.4 + "px"
          );
      });
  ...
  updateGeographicalChart() {
    ...
    this.statesData.forEach(
      (stateData) => (this.stateCounts[stateData.label] = 0)
    );
    this.statesData.forEach(
      (stateData) => (this.weightedStateCounts[stateData.label] = 0)
    );

    this.currentData.forEach(
      (row) => (this.stateCounts[this.stateMapping[row.State]] += 1)
    );
    ...
    var counts = this.weightedByPopulation
      ? this.weightedStateCounts
      : this.stateCounts;
    ...
    this.stateCountStats.mean =
      parseFloat(this.stateCountStats.total) / Object.entries(counts).length;
    ...
    this.stateCountStats.absolute_mean =
      parseFloat(this.stateCountStats.absolute_total) /
      Object.entries(this.stateCounts).length;
    ...
    var colorScaleGradient = this.geographicalSvg
      .append("defs")
